...
#define MAXDEMOD 6 /* Demods: CCIR, EEA, EIA, ZVEII, ZVEIII, ZVEIII */
...
typedef struct {
	...
	unsigned int fbuf_cnt;
    	short sbuf[160];
    	float fbuf[320];
	const struct demod_param *dem[MAXDEMOD];
	struct demod_state dem_st[MAXDEMOD];
} selcall_decoder_t;
